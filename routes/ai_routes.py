...
2) Создать массив шагов (steps), где каждый шаг описан полями "title", "description", "date".
3) Если в какой-то день уже 3 или более задач, старайся НЕ использовать эту дату — ищи менее загруженный день.
4) При разбиении цели на шаги учитывай логичную последовательность и устанавливай адекватные промежутки между датами — избегай слишком плотного или слишком растянутого графика. Почти никогда нельзя ставить шаги подряд! Лучше растянуть график шагов, нежели сбить в одну кучу!
5) Старайся избегать выходных дней 
6) Обязательно ставь шаги на даты, которые стоят после текущей даты, можешь ставить первый шаг на текущую дату.
7) goal_title должен быть коротким и содержательным.
8) Формат даты: YYYY-MM-DD.
9) НЕЛЬЗЯ добавлять пояснения или текст вне JSON.
10) Верни результат СТРОГО в виде JSON, без комментариев, соблюдая следующую структуру:
